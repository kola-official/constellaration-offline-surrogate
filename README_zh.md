...
模型学到的“可行性”至多来自连续违反量的插值或外推。该子集中不存在真正的可行平衡样本可供学习。
...
- 排行榜行确立**最终审计边界与得分**。公开表不给出各提交者的搜索历史与
  VMEC++ 调用次数。私有搜索轨迹不在本仓库范围之内。
- 论文 ALM-NGOpt 在优化内环调用物理，算力预算大。本工作采用**离线代理搜索 +
  本地固定审计预算**。两套协议的预算与评价流程不同。
- 本项目给出“离线代理排序在何种条件下可信”的方法学诊断。
