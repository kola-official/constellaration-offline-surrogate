...
本仓库建立在官方 ConStellaration 生态之上，而不是替代它：

| 资源 | 在本项目中的角色 |
|---|---|
| [官方数据集](https://huggingface.co/datasets/proxima-fusion/constellaration) | 离线训练与诊断的数据来源 |
| [官方代码 / 正演接口](https://github.com/proximafusion/constellaration) | 问题定义、评分规则与 VMEC++ 审计入口 |
| [设计排行榜](https://huggingface.co/spaces/proxima-fusion/constellaration-bench) | 公开展示经官方评价的 Problem 2 边界与得分 |
| 论文 ALM-NGOpt 基线 | 在线物理优化的外部参考（非同等预算复现） |

对应关系如下：

- **官方评价具有最终权威。** 可行性与 score 由 ConStellaration 正演模型定义。
  代理输出仅为预测量；VMEC++ / 官方评价器输出为审计物理量。
- **排行榜记录确立“最终审计边界 + 得分”。** 公开榜单不披露各提交者的候选
  生成历史，也不披露搜索过程中的 VMEC++ 调用次数。因此本项目不重构、也不
  与私有搜索轨迹做排名对照。
- **论文 ALM-NGOpt 是在线基线。** 其优化内环反复调用物理评价，并在较大算力
  预算下报告 Problem 2 可行得分。本仓库研究的是另一套协议：**离线代理搜索 +
  本地固定审计预算**。二者不是同等预算对比。
- **本项目补充的内容。** 在所用 Nfp=3 子集零官方可行训练标签的条件下，定量
  刻画代理可信边界、约束违反地板、信任域塌缩与随机预筛对照。主要产出是关于
  “离线代理排序何时可信、何时失效”的方法学证据，而不是宣称新的榜单最优解。

一句话区分：排行榜回答“在官方指标下，哪条已审计边界得分最高？”；本仓库回答
“仅利用公开数据、在固定预算离线代理流程中，能学到什么、会在何处失败？”

## 未来方向

后续优先方向与技术报告一致，并保持评价口径不变（官方得分对累积高保真调用
次数）：

1. **稀疏反馈 / 混合主动学习。** 代理只负责提议批次；VMEC++ 审计；回填标签；
   重训；绘制最优官方得分随累积物理调用的曲线。该方向属于混合工作流，不是
   纯离线搜索。
2. **可行侧或近可行侧数据获取。** 当前主要缺口是官方可行 / 近可行标签的覆盖，
   而不是仅在现有不可行主导数据上扩大模型容量。
3. **用中间物理量改进排序。** 推理输入仍保持 Fourier 系数；将 wout 派生或
   多指标头用于 VMEC 稳定性过滤与更保守的候选排序，而不是单独当作优化目标。
4. **谨慎的跨域迁移。** Cross-Nfp 预训练已改善监督指标；更广的迁移（如相关
   QA/QH 语料）必须保持 Problem 2 的 vacuum 定义与官方评分一致。
5. **边界表征研究。** 近轴展开、学习型潜边界、谱带分解等可能改变可行域几何，
   本发布刻意不纳入范围。
6. **工程整理。** 共享包提取、更完整的单元测试（特征顺序、约束归一化、
   boundary round-trip、审计预算计数）以及干净环境下的 smoke run。

已完成阶段与开放方向的证据见中文报告
[`presentations/advisor_report/report_cn.md`](presentations/advisor_report/report_cn.md)
第七部分，以及 [`docs/METHODOLOGY.md`](docs/METHODOLOGY.md)。
