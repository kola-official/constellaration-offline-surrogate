...
4. **表示升级改善监督，不自动带来可行性。** 15 指标、wout 中间监督、跨 Nfp
   ...
   官方可行候选**。
5. **主要产出是方法学诊断证据**，而不是宣称公开设计排行榜上的新最优解。跨越
   可行线更可能需要可行侧/近可行侧高保真标签（混合主动学习）。
...
**relaxed55** 是按论文风格**放宽阈值**得到的近可行种子，**不是**官方可行集，
且不进入任何监督 train/val/test；仅作生成先验。
...
| 资源 | 本项目中的角色 |
|---|---|
| [数据集](https://huggingface.co/datasets/proxima-fusion/constellaration) | 离线训练与诊断 |
| [代码 / 正演](https://github.com/proximafusion/constellaration) | 问题定义、评分、审计接口 |
| [设计排行榜](https://huggingface.co/spaces/proxima-fusion/constellaration-bench) | 公开展示已审计边界与得分 |
| 论文 ALM-NGOpt 基线 | 外部**在线**物理优化参考 |

- 官方评价具有最终权威。代理值为预测量；VMEC++ 值为审计物理量。
- 排行榜行确立**最终审计边界与得分**，不公开各提交者的搜索历史与 VMEC++
  调用次数；本项目不重构私有轨迹。
- 论文 ALM-NGOpt 在优化内环调用物理，算力预算大。本工作是**离线代理搜索 +
  本地固定审计预算**，二者不是同等预算对比。
- 本项目贡献的是“离线代理排序何时可信”的方法学证据，而非宣称新的榜单最优解。

**排行榜问题：** 在官方指标下，哪条已审计边界得分最高？  
**本仓库问题：** 仅用公开数据、在固定预算离线代理流程中，能学到什么、会在何处
失败？
...
1. 混合主动学习：代理提议、VMEC 审计、回填重训；绘最优官方得分对累积物理
   调用曲线。
2. 获取可行侧 / 近可行侧高保真标签。
3. 将中间物理量用作排序**过滤**，而非唯一优化目标。
4. 更广迁移须对齐 Problem 2 vacuum 定义与官方评分。
5. 边界表征研究（近轴、学习潜变量、谱带）——本发布刻意不纳入。
