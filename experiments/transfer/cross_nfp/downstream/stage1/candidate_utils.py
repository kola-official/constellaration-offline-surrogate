...
def load_model_bundle(device: str = "cuda:0") -> dict[str, Any]:
    model_dir = Path(os.environ.get("CROSS_NFP_MODEL_DIR", OUTPUT_DIR / "models" / "surrogate_ensemble"))
    scalers = np.load(model_dir / "scalers.npz", allow_pickle=True)
    x_mean = scalers["x_mean"].astype(np.float32)
    x_std = np.where(scalers["x_std"].astype(np.float32) < 1e-8, 1.0, scalers["x_std"].astype(np.float32))
    y_mean = scalers["y_mean"].astype(np.float32)
    y_std = scalers["y_std"].astype(np.float32)
    violation_mean = float(
        np.asarray(scalers["violation_mean"] if "violation_mean" in scalers.files else np.array(0.0)).item()
    )
    violation_std = float(
        np.asarray(scalers["violation_std"] if "violation_std" in scalers.files else np.array(1.0)).item()
    )
    feasibility_threshold = float(
        np.asarray(
            scalers["feasibility_threshold"]
            if "feasibility_threshold" in scalers.files
            else np.array(DEFAULT_FEASIBILITY_THRESHOLD)
        ).item()
    )
    feasibility_temperature = float(
        np.asarray(
            scalers["feasibility_temperature"]
            if "feasibility_temperature" in scalers.files
            else np.array(DEFAULT_FEASIBILITY_TEMPERATURE)
        ).item()
    )
    labels = [str(x) for x in scalers["labels"]] if "labels" in scalers.files else list(PROBLEM2_LABELS)
    feature_cols = [str(x) for x in scalers["feature_columns"]]
    geometry_feature_cols = (
        [str(x) for x in scalers["geometry_feature_columns"]]
        if "geometry_feature_columns" in scalers.files
        else feature_cols[: len(x_mean)]
    )
    nfp_condition_included = bool(
        np.asarray(
            scalers["nfp_condition_included"]
            if "nfp_condition_included" in scalers.files
            else np.array(len(feature_cols) > len(x_mean))
        ).item()
    )
    nfp_scale = float(
        np.asarray(scalers["nfp_scale"] if "nfp_scale" in scalers.files else np.array(3.0)).item()
    )
    ...
    return {
        ...
        "geometry_feature_cols": geometry_feature_cols,
        "nfp_condition_included": nfp_condition_included,
        "nfp_scale": nfp_scale,
        "default_nfp": 3.0,
...
def predict_ensemble(bundle: dict[str, Any], x_raw: np.ndarray, batch_size: int = 8192) -> dict[str, Any]:
    ...
    if x_raw.ndim == 1:
        x_raw = x_raw[None, :]
    geometry_dim = int(bundle["x_mean"].shape[0])
    feature_dim = len(bundle["feature_cols"])
    if x_raw.shape[1] == geometry_dim:
        x_geometry = x_raw
        condition = None
    elif x_raw.shape[1] == feature_dim:
        x_geometry = x_raw[:, :geometry_dim]
        condition = x_raw[:, geometry_dim:]
    else:
        raise ValueError(
            f"Expected {geometry_dim} geometry features or {feature_dim} conditioned features, "
            f"got {x_raw.shape[1]}."
        )
    x_scaled = (x_geometry - bundle["x_mean"]) / bundle["x_std"]
    if bundle.get("nfp_condition_included", False):
        if condition is None:
            condition_value = float(bundle.get("default_nfp", 3.0)) / float(bundle.get("nfp_scale", 3.0))
            condition = np.full((x_scaled.shape[0], feature_dim - geometry_dim), condition_value, dtype=np.float32)
        x_scaled = np.concatenate([x_scaled, condition.astype(np.float32)], axis=1)
    if x_scaled.shape[1] != feature_dim:
        raise ValueError(f"Model expects {feature_dim} features after conditioning, got {x_scaled.shape[1]}.")
    ...
    with torch.no_grad():
        for model in bundle["models"]:
            ...
            for start in range(0, tensor.shape[0], batch_size):
                pred_y, pred_v = model(tensor[start : start + batch_size])
                ys.append(pred_y.cpu().numpy())
                vs.append(pred_v.cpu().numpy())
