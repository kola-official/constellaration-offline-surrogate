...
from common_stage2 import OUTPUT_DIR, apply_thread_environment, ensure_output_dirs, load_config, parse_args, write_json
...
def load_candidates(limit: int) -> list[dict[str, Any]]:
    ...
    for method_id, filename in CANDIDATE_FILES.items():
        ...
        method_records = [json.loads(line) for line in path.read_text().splitlines() if line.strip()]
...
def load_benchmark_candidates() -> list[dict[str, Any]]:
    records = []
    for _, filename in CANDIDATE_FILES.items():
        path = OUTPUT_DIR / "candidates" / filename
        method_records = [json.loads(line) for line in path.read_text().splitlines() if line.strip()]
        method_records = sorted(method_records, key=lambda item: item["rank_before_audit"])
        if method_records:
...
def main() -> None:
    ...
    if args.benchmark_only:
        ...
        write_json(OUTPUT_DIR / "run_summary" / "vmec_benchmark_stage2.json", benchmark)
    ...
    write_jsonl(OUTPUT_DIR / "vmec_audit" / "audit_stage2.jsonl", audit_results)
    manifest = {
    ...
    write_json(OUTPUT_DIR / "vmec_audit" / "audit_manifest_stage2.json", manifest)
